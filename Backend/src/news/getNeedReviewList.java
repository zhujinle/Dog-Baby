...
@WebServlet(name = "getNeedReviewList", value = "/getNeedReviewList")
public class getNeedReviewList extends HttpServlet {
    @Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if(!Objects.equals(uid, "")){
            String result = dao.getNewsListByType("2",uid);
            response.setContentType("text/json; charset=utf-8");
            response.getWriter().write("{\"statusCode\": 200, \"msg\": \"读取成功\","+"\"data\":"+result+"}");
            return;
        }
        String result = dao.getNewsListByType("2",null);
        response.setContentType("text/json; charset=utf-8");
        response.getWriter().write("{\"statusCode\": 200, \"msg\": \"读取成功\","+"\"data\":"+result+"}");
    }
}